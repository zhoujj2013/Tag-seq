...
# create gRNA.fa
open OUT,">","$conf{OUTDIR}/grna.fa" || die $!;
print OUT ">grna\n";
print OUT "$conf{GRNA}\n";
close OUT;
...
print OUT "GRNA\t$conf{GRNA}\n";
print OUT "GRNA_LENGTH\t".length($conf{GRNA})."\n";
...
mkdir "$conf{OUTDIR}/find.target" unless(-d "$conf{OUTDIR}/find.target");
chdir "$conf{OUTDIR}/find.target";
