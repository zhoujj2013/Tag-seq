...
while(<IN>){
	...
	if($gap > 0){
		...
		if($ge > $ref_length - $spacer_sed_mis && (($ge - $gs)/$ref_length) > $match_perc){
			my $rseq_orign = $rseq;
			$rseq_orign =~ s/-//g;
			$rseq_orign = uc($rseq_orign);
			#print "$rseq_orign\n";
			my @p;
			while($fwseq{$rid} =~ /$rseq_orign/g){
				push @p,$-[0];
			}
			
			my $align_end = $p[0] + length($rseq_orign);
			#my $extend_end = $align_end + 10;
			my $pam_flag = 0;
			for(my $k = 0; $k < $pam_dis; $k++){
				if($align_end+$k > length($fwseq{$rid})-3){
					next;
				}
				my $pam = substr($fwseq{$rid},$align_end+$k,3);
				if(exists $pam_dict{$pam}){
					$pam_flag = 1;
				}
			}
...
while(<IN>){
	...
	if($gap > 0){
		...
		if($ge > $ref_length - $spacer_sed_mis && (($ge - $gs)/$ref_length) > $match_perc){
			...
			my $pam_flag = 0;
			for(my $k = 0; $k < $pam_dis; $k++){
				if($align_end+$k > length($rvseq{$rid})-3){
                                        next;
                                }
				my $pam = substr($rvseq{$rid},$align_end+$k,3);
				if(exists $pam_dict{$pam}){
					$pam_flag = 1;
				}
			}
